...
// This disables Vercel's default parser so we can verify the raw Discord signature
...
module.exports = async (req, res) => {
  ...
  // 1. Verify the Request from Discord
  // (Requires PUBLIC_KEY to be set in Vercel Environment Variables)
  ...
  if (interaction.type === InteractionType.MESSAGE_COMPONENT && interaction.data.custom_id === "levelCalcButton") {
  ...
  if (interaction.type === InteractionType.MODAL_SUBMIT && interaction.data.custom_id === "levelCalcModal") {
    // Extract values from the raw JSON payload
    ...
    const sendError = (msg) => res.send({
      ...
      data: { content: msg, flags: 64 } // flag 64 = ephemeral
    });
